...
export const Scene3D: React.FC<Props> = ({
  ...
  // ── Mise à jour SVG impérative pendant le drag ───────────────────────────
  // Met à jour les attributs SVG directement sans passer par React
  const updateSvgElement = useCallback((el: MixElement) => {
    const svg = svgRef.current;
    if (!svg) return;
    const { pos, sx, sy, r, floor, dotX } = computeScreenPos(el);
    const id = el.id;

    // Groupe principal de l'élément
    const gEl = svg.querySelector(`[data-el="${id}"]`);
    if (!gEl) return;

    // Ombre sol
    const shadow = gEl.querySelector('.el-shadow') as SVGEllipseElement | null;
    if (shadow) {
      shadow.setAttribute('cx', String(floor.sx));
      shadow.setAttribute('cy', String(floor.sy));
      shadow.setAttribute('rx', String(r * 0.62 * (1 - pos.z * 0.3)));
      shadow.setAttribute('ry', String(r * 0.15 * (1 - pos.z * 0.3)));
    }

    // Ligne sol
    const line = gEl.querySelector('.el-line') as SVGLineElement | null;
    if (line) {
      line.setAttribute('x1', String(floor.sx));
      line.setAttribute('y1', String(floor.sy));
      line.setAttribute('x2', String(sx));
      line.setAttribute('y2', String(sy + r * 0.82));
    }

    // Halos sélection (s'il est sélectionné)
    const halo1 = gEl.querySelector('.el-halo1') as SVGCircleElement | null;
    const halo2 = gEl.querySelector('.el-halo2') as SVGCircleElement | null;
    if (halo1) { halo1.setAttribute('cx', String(sx)); halo1.setAttribute('cy', String(sy)); halo1.setAttribute('r', String(r + 10)); }
    if (halo2) { halo2.setAttribute('cx', String(sx)); halo2.setAttribute('cy', String(sy)); halo2.setAttribute('r', String(r + 17)); }

    // Glow
    const glow = gEl.querySelector('.el-glow') as SVGCircleElement | null;
    if (glow) { glow.setAttribute('cx', String(sx)); glow.setAttribute('cy', String(sy)); glow.setAttribute('r', String(r * 1.55)); }

    // Anneau de séparation
    const ring = gEl.querySelector('.el-ring') as SVGCircleElement | null;
    if (ring) { ring.setAttribute('cx', String(sx)); ring.setAttribute('cy', String(sy)); ring.setAttribute('r', String(r + 3)); }

    // Cercle principal
    const circle = gEl.querySelector('.el-circle') as SVGCircleElement | null;
    if (circle) {
      circle.setAttribute('cx', String(sx));
      circle.setAttribute('cy', String(sy));
      circle.setAttribute('r', String(r));
      circle.setAttribute('opacity', String(pos.opacity));
    }

    // Icône
    const icon = gEl.querySelector('.el-icon') as SVGTextElement | null;
    if (icon) {
      icon.setAttribute('x', String(sx));
      icon.setAttribute('y', String(sy));
      icon.setAttribute('font-size', String(Math.max(r * 0.86, 11)));
    }

    // Nom
    const name = gEl.querySelector('.el-name') as SVGTextElement | null;
    if (name) {
      name.setAttribute('x', String(sx));
      name.setAttribute('y', String(sy + r + 14));
    }

    // Badge %
    const badge = gEl.querySelector('.el-badge') as SVGTextElement | null;
    if (badge) {
      badge.setAttribute('x', String(sx + r * 0.52));
      badge.setAttribute('y', String(sy - r * 0.58));
      badge.textContent = `${Math.round((1 - pos.z) * 100)}%`;
    }

    // Dot dans la barre de profondeur
    const dotG = svg.querySelector(`[data-dot="${id}"]`);
    if (dotG) {
      const dotText = dotG.querySelector('.dot-text') as SVGTextElement | null;
      if (dotText) {
        dotText.setAttribute('x', String(dotX));
      }
      const dotHalo = dotG.querySelector('.dot-halo') as SVGCircleElement | null;
      if (dotHalo) {
        dotHalo.setAttribute('cx', String(dotX));
      }
    }
  }, []);
